    ...
    JsonFetch.then(respose => respose.json()).then(data => {
        ...
        data.forEach(userData => {
            output += `<div class="bg-white p-3 sm:p-4 md:p-6 mb-3 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                       ...
                       <h3 class="text-sm sm:text-base text-gray-600">Street Number : ${userData.address.streetNumber}</h3>
                       <h3 class="text-sm sm:text-base text-gray-600">Street : ${userData.address.Street}</h3>
                       <h3 class="text-sm sm:text-base text-gray-600">District : ${userData.address.district}</h3>
                       </div>
                       `
        });
